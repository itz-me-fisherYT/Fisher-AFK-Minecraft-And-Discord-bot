...
function createBot() {
  ...
  bot.chat(`Bye come back later! ${player.username}.`)
});
  ...
  const bossName = ["itz_me_fisherYT"];

  bot.on("chat", async function(username, message) {
    if (!bossName.includes(username)) return;
    ...
    if (msg.startsWith("!")) {
      ...
      if (text === "boss") {
        bot.chat(`my owner is > ${bossName}`);
        bot.chat("i am his slave!");
        bot.chat("itz_me_fisherYT i am going to tpa! {if there is /tpa}");
        bot.chat("/tpa itz_me_fisherYT");
      }
      if (text === "discord") {
        bot.chat("join the discord! https://discord.gg/j9WJ9F8KnM !");
      }
      if (text === 'leave') {
        bot.chat("ill rejoin in 10 seconds");
        bot.quit();
      }
      if (text === 'tips') {
        bot.chat("remember to see your bed! there is not /sethome, /home");
        bot.chat("no /tpa no /enderchest");
      }
      if (text === 'help') {
        bot.chat("my commands are !boss, !discord, !tips, and !leave. :D");
      }
    }
  });
  ...
  bot.on('end', () => {
    console.log(chalk.blue('Disconnected, attempting to reconnect in 10 seconds...'));
    setTimeout(() => {
